...
- **Blank Notification Mode**: Strips the notification title and text entirely, leaving only the clean status bar icon for minimalists.
- **Optimized Layout**: Download speed now displays before Upload speed for quicker readability.
- **OLED Theme & Custom Colors**: Pure black theme support for OLED screens, with a default gray status pill that can be fully customized.
...
Because this app intentionally spoofs another application's package name and signature, **Google Play Protect will flag it.** When installing the APK, Play Protect may warn you that the app did not come from the Play Store and has a different signature. This is the cryptographic security working exactly as intended. Just click **"More details" -> "Install anyway"**.

**Do not update this app via the Play Store**, or your speed meter will be overwritten by an actual Korean ride-hailing app.
